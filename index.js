...
book.get("/viewMyRecipes/:username", async function(request,response){
   ...
   response.send(`<style>#back{background-color:blue; color:yellow;} body{background-color:dodgerblue; } ul{list-style-type:none; background-color:white; width:100px;}li{ border:1px solid black; text-align:center; width:100px;}</style><button onclick="window.location.href='/'" id="back">Back</button>${GetRecipes(myRecipes)}`);
});
